...
## Current phase
The approved P0 is a WhatsApp-first growth agent for small businesses with an additional guided customer-facing Axcas Studio. It must infer a constrained business type (`home_bakery`, `tailor`, `tutor`, `salon`, `home_service`, `retailer`, or `other`) instead of asking the merchant to select a business template. Studio may ask only whether the merchant wants Website, Reels, or Both, then offers code-owned site layouts and structured reel-layer controls. Complete the smallest path from merchant voice/photos/offerings or a WhatsApp-linked Studio project to an approved, verified `workers.dev` product-or-service site, tracked WhatsApp CTA, one consented Vapi batch, one approved reel, and a metrics/improvement report.
...
There is no operator dashboard, blank-canvas code editor, payment flow, scraped lead source, autonomous publishing, or outbound call without exact batch approval.

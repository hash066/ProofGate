...
describe("AWS reel guardian", () => {
  it("claims only an approved job, downloads its exact assets, uploads the verified MP4, and completes it", async () => {
    ...
    const render = vi.fn(async () => new Uint8Array([0, 0, 0, 24, 102, 116, 121, 112, 1, 2, 3]));
    ...
    expect(calls.at(-1)).toEqual({ url: "https://axcas.example/internal/reel-result", method: "POST" });
  });
});

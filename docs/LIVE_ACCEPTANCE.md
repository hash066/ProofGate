...
- [x] The named Cloudflare Worker, KV namespace, production Convex configuration, Meta POST/send secrets, and durable AWS API Gateway/SQS Hermes origin have control-plane receipts.
...
- [x] AWS Hermes reports exactly `v0.18.2`, the repository skill is linked/enabled, and the official Cloud adapter is configured. Its health returns 200 with signature/verify configuration present.
- [x] Hermes is running on AWS behind a stable authenticated API Gateway/SQS ingress. The temporary quick tunnel is disabled.
- [x] The Vapi squad was inspected in-provider: consent member artifacts are off, qualification recording/logging/transcript are on, and both assistants send only signed end-of-call reports to the Worker. No call has been placed.
